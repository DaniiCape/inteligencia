...
public class AEstrella extends PApplet {
    ...
    class Mosaico{
        ...
        void cambiaMosaico(){
            this.situacion = Situacion.EN_SOLUCION;
        }
    ...
    class Algoritmo {
        ...
        void expandeNodoSiguiente() {
            NodoBusqueda meta = new NodoBusqueda(estadoFinal);
            while(this.nodoActual.compareTo(meta) != 0){
            NodoBusqueda a = listaAbierta.remove();
            listaCerrada.put(nodoPrevio.estado, nodoPrevio.estado);
            
            }
            resuelto = true;
            listaCerrada.forEach();
            /**
              * IMPLEMENTACION
